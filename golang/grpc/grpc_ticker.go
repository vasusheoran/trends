...
func (t TickerServer) handleUpdates() {
	...
	for range t.ch {
		level.Info(t.logger).Log("msg", "Handling updates", "subscribers", len(t.subscribers))
...
func (t *TickerServer) GetSummary(src client.Ticker_GetSummaryServer) error {
	for {
		...
		level.Info(t.logger).Log("msg", "Successfully subscribed to GetSummary", "req", req.String())
	}
	return nil
}
